...
model.fit(x=train_padded,y=y,validation_split=.1,epochs=1,batch_size=64)

test_pred = model.predict(x=test_padded)
list_classes = ["toxic", "severe_toxic", "obscene", "threat", "insult", "identity_hate"]
sample_submission = pd.read_csv('sample_submission.csv')
sample_submission[list_classes] = test_pred
sample_submission.to_csv('LSTM-newSub.csv', index=False)





#print(check_output(["ls", "~/.kaggle/competitions/jigsaw-toxic-comment-classification-challenge"]).decode("utf8"))
